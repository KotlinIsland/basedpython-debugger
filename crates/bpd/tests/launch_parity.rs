...
fn a_program_that_reads_its_own_environment_finds_no_debugger_in_it() {
    ...
    for form in EVERY_FORM {
        ...
        // every variable bpd launches through is `BPD_…`, and that prefix is
        // what this looks for rather than the three letters anywhere in a
        // name. **windows upper-cases an environment variable's name**, so
        ...
        assert!(
            leaked.is_empty(),
            "the program could read a launch parameter as {form:?}: {leaked:?}"
        );
    }
}
...
/// first**: written raw, `C:\Users\…` reached the parser as `\U` and was
/// refused by name — which is the parser doing its job about a map this test
/// wrote badly
fn in_the_map(path: &Path) -> String {
    path.display()
        .to_string()
        .replace('\\', r"\\")
        .replace('"', "\\\"")
...
fn write_source_map(fixture: &Fixture, source: &Path) {
    ...
    std::fs::write(
        ...
        format!(
            ...
            generated = in_the_map(&generated),
            source = in_the_map(source),
            table = lines.join(", "),
            by = digest(source),
            py = digest(&generated),
        ),
...
/// `#[cfg(unix)]` because **windows has no `fork`**, and bpd refuses
/// `--debug-children` there by name: every child a program starts on that
/// platform is a fresh interpreter with none of this process's memory in it, so
/// there is nothing to inherit a session and no `os.register_at_fork` to hand
/// one over in. the refusal is the behaviour, and a debuggee that cannot be
/// asked for this has no fingerprint from it to compare
#[cfg(unix)]
#[test]
...
///
/// `#[cfg(unix)]` for the reason the test above it is: `--debug-children` is
/// refused on windows, so there is nothing there to compare
#[cfg(unix)]
#[test]
    ...
    for form in EVERY_FORM {
        let fixture = Fixture::new(
            ...
            "import os\n\
             ...
             other = ['cmd', '/c', 'echo'] if os.name == 'nt' else ['/bin/echo']\n\
             subprocess.run(other, check=True, capture_output=True)\n\
             print('\\n'.join(seen))\n\
